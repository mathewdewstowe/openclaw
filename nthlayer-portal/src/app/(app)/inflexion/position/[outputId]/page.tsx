...
export default async function PositionOutputPage({ params }: { params: Promise<{ outputId: string }> }) {
  const user = await getCurrentUser();
  if (!user) redirect("/login");
  const { outputId } = await params;
  const output = await db.output.findUnique({ where: { id: outputId }, include: { company: { select: { name: true } } } });
  if (!output) notFound();
